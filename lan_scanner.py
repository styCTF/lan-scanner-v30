...
def _parse_ipconfig_windows():
    ...
    # 获取系统控制台输出代码页 (中文=936, 英文=437, 日文=932 等)
    try:
        oem_cp = ctypes.windll.kernel32.GetConsoleOutputCP()
    except Exception:
        oem_cp = 936
    ...
    for line in output.splitlines():
        ...
        # IPv4 地址 — "IPv4" 在所有语言中保持不变
        if "IPv4" in line:
            if ":" in line:
                current_ip = line.split(":")[-1].strip()
            saw_ipv4 = True

        # 子网掩码 — 值总是点分十进制 (如 255.255.255.0), 语言无关
        # saw_ipv4 保证只匹配紧跟 IPv4 地址后的掩码值, 避免误匹配网关等
        if saw_ipv4 and ":" in line:
...
def get_netbios_name(ip: str, timeout: float = 1.0) -> str:
    ...
    try:
        ...
        # 查找 <00> UNIQUE 类型的 NetBIOS 名称 (机器名)
        for line in output.splitlines():
...
def print_results(devices: list[Device], network_str: str, local_ip: str = "", gateway: str = ""):
    ...
    for d in devices:
        ...
        vendor_display = f"{vendor} / {dev_type}" if dev_type else vendor
        name_display = d.hostname or "-"
        vendor_display = vendor_display or "-"
        if d.ip == local_ip and d.ip == gateway:
            ...
            tag = "在线"
        print(f"  {d.ip:<16}  {d.mac or '-':<18}  {name_display:<20}  {vendor_display:<20}  {tag}")
...
def export_csv(devices: list[Device], filename: str = ""):
    ...
    with open(filename, "w", encoding="utf-8-sig", newline="") as f:
        ...
        for d in devices:
            vendor = mac_to_vendor(d.mac) if d.mac else ""
            dev_type = vendor_device_type(vendor) if vendor else ""
            writer.writerow([d.ip, d.mac or "", d.hostname, vendor, dev_type])
