#!/usr/bin/env python3
"""局域网 IP 扫描器 v3.0 —— 扫描并列出同一网段内的所有在线设备。支持 GUI 和命令行两种模式。"""
...
def parse_ping_ttl(ip: str, timeout: float = 0.8) -> int:
    """Ping 一个 IP 并解析返回的 TTL 值。失败返回 0。"""
    import re
    param = "-n" if IS_WINDOWS else "-c"
    timeout_param = "-w" if IS_WINDOWS else "-W"
    timeout_val = str(int(timeout * 1000)) if IS_WINDOWS else str(max(1, int(timeout)))
    ...
    try:
        ...
        output = result.stdout.decode(f"cp{_get_windows_cp()}" if IS_WINDOWS else "utf-8", errors="ignore")
        if result.returncode != 0:
            return 0
        # Windows: "TTL=64" / Linux: "ttl=64"
        m = re.search(r"[Tt][Tt][Ll][\s=]*(\d+)", output)
        if m:
...
def guess_os(ttl: int, vendor: str = "", hostname: str = "") -> str:
    """根据 TTL + 厂商 + 主机名综合推断操作系统。

    返回值为 os 名称字符串, 空串表示无法判断。
    """
    vendor_lower = vendor.lower()
    ...
    if ttl >= 120:
        ...
        # 100-119: 可能是 Windows (修改了默认 TTL)
        return "Windows"
    elif 55 <= ttl <= 64:
        # 60-64: Linux/macOS/Android 范围
        if any(k in vendor_lower for k in ("apple", "iphone", "ipad", "macbook")):
            return "macOS / iOS"
        if any(k in hostname_lower for k in ("iphone", "ipad", "macbook", "mac-")):
            return "macOS / iOS"
        if "android" in hostname_lower:
            return "Android"
        if any(k in vendor_lower for k in ("raspberry", "ubuntu", "debian")):
            return "Linux"
        # Apple 厂商的设备在这个 TTL 范围 = iOS/macOS
        ...
        return "网络设备 (Cisco/IOS)"
    elif ttl > 64 and ttl < 100:
        return "Linux"

    # ── TTL 缺失或异常, 用 MAC 厂商兜底 ──
    ...
    if "raspberry" in vendor_lower:
        return "Linux"
    if any(k in vendor_lower for k in ("cisco", "mikrotik", "juniper", "aruba", "zyxel", "ubiquiti")):
        return "网络设备"

    # ── 主机名启发 ──
    if hostname.startswith("DESKTOP-") or hostname.startswith("WIN-"):
        return "Windows"
    if "android" in hostname_lower:
        return "Android"
...
def scan_network(network_str: str, max_workers: int = 64,
                on_device_found=None, on_progress=None, stop_check=None) -> list[Device]:
    """扫描整个网段, 返回在线设备列表。

    on_device_found(device): 每发现一个设备时调用 (GUI 回调)
    on_progress(current, total, found): 进度回调
    stop_check(): 返回 True 时中止扫描
    """
    ...
    if arp_devices:
        ...
        for d in arp_devices:
            ...
            d.os_type = guess_os(parse_ping_ttl(d.ip), d.vendor, d.hostname)
            print(f"      {d.ip:<16}  {d.mac or '-':<18}  {d.hostname or '-':<20}  {d.vendor_display}  {d.os_type}")
            if on_device_found:
                on_device_found(d)
    ...
    def scan_one(ip_str: str):
        ...
        if ip_str not in devices and is_host_alive(ip_str):
            ...
            ttl = parse_ping_ttl(ip_str)
            os_type = guess_os(ttl, vendor, hostname)
            d = Device(ip=ip_str, mac=mac, hostname=hostname, vendor=vendor, dev_type=dev_type, os_type=os_type)
            with lock:
                devices[ip_str] = d
            os_str = f"  {os_type}" if os_type else ""
            print(f"    + {d.ip:<16}  {d.mac or '-':<18}  {hostname or '-':<20}  {d.vendor_display}{os_str}")
            if on_device_found:
                on_device_found(d)
        ...
        with lock:
            finished[0] += 1
            if on_progress:
                on_progress(finished[0], total, len(devices))
            if finished[0] == total:
...
def print_results(devices: list[Device], network_str: str, local_ip: str = "", gateway: str = ""):
    ...
    header = f"  {'IP 地址':<16}  {'MAC 地址':<18}  {'主机名':<16}  {'厂商 / 设备类型':<20}  {'操作系统':<14}  状态"
    print(header)
    print(f"  {'-' * 16}  {'-' * 18}  {'-' * 16}  {'-' * 20}  {'-' * 14}  ----")

    for d in devices:
        name_display = d.hostname or "-"
        os_display = d.os_type or "-"
        if d.ip == local_ip and d.ip == gateway:
            ...
            tag = "在线"
        print(f"  {d.ip:<16}  {d.mac or '-':<18}  {name_display:<16}  {d.vendor_display:<20}  {os_display:<14}  {tag}")
...
def export_csv(devices: list[Device], filename: str = ""):
    ...
    with open(filename, "w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["IP 地址", "MAC 地址", "主机名", "厂商", "操作系统", "设备类型"])
        for d in devices:
            writer.writerow([d.ip, d.mac or "", d.hostname, d.vendor, d.os_type, d.dev_type])
...
class ScannerApp:
    """局域网 IP 扫描器 GUI (tkinter)。"""

    def __init__(self):
        self.root = tk.Tk()
        self.root.title("局域网 IP 扫描器 v3.0")
        ...
        self.networks: list[tuple] = []  # [(iface, ip, net_str), ...]
    ...
    def _build_ui(self):
        ...
        pw.pack(fill=tk.BOTH, expand=True, padx=10, pady=(10, 4))

        # 左侧：控制面板 + IP 占用图
        left_frame = ttk.Frame(pw, width=300)
        left_frame.pack_propagate(False)
        pw.add(left_frame, weight=0)
        self._build_control_panel(left_frame)
        self._build_ip_map(left_frame)
    ...
    def _build_control_panel(self, parent):
        ...
        # 网卡选择
        ttk.Label(parent, text="网络接口", font=("", 10, "bold")).pack(anchor=tk.W, pady=(14, 4))
        self.iface_combo = ttk.Combobox(parent, state="readonly")
        self.iface_combo.pack(fill=tk.X)
        self.iface_combo.bind("<<ComboboxSelected>>", self._on_iface_selected)

        # 网段显示
        ttk.Label(parent, text="扫描网段", font=("", 10, "bold")).pack(anchor=tk.W, pady=(14, 4))
        self.network_var = tk.StringVar()
        ttk.Entry(parent, textvariable=self.network_var, state="readonly").pack(fill=tk.X)

        # 本机 IP
        ttk.Label(parent, text="本机 IP", font=("", 10, "bold")).pack(anchor=tk.W, pady=(14, 4))
        self.local_ip_var = tk.StringVar()
        ttk.Entry(parent, textvariable=self.local_ip_var, state="readonly").pack(fill=tk.X)

        # 线程数
        ttk.Label(parent, text="并发线程数", font=("", 10, "bold")).pack(anchor=tk.W, pady=(14, 4))
        self.threads_var = tk.StringVar(value="128")
        ttk.Spinbox(parent, from_=8, to=512, increment=8, textvariable=self.threads_var, width=8).pack(anchor=tk.W)

        # 按钮
        btn_frame = ttk.Frame(parent)
        btn_frame.pack(fill=tk.X, pady=(16, 6))
        ...
        # 进度条
        ttk.Label(parent, text="扫描进度", font=("", 10, "bold")).pack(anchor=tk.W, pady=(14, 4))
        self.progress_var = tk.IntVar()
        self.progress_bar = ttk.Progressbar(parent, variable=self.progress_var, mode="determinate")
        self.progress_bar.pack(fill=tk.X)
        self.progress_label = ttk.Label(parent, text="就绪")
        self.progress_label.pack(anchor=tk.W, pady=(2, 0))

        # 刷新网卡按钮
        ttk.Button(parent, text="刷新网卡列表", command=self._load_interfaces).pack(fill=tk.X, pady=(16, 0))

    def _build_ip_map(self, parent):
        """在左侧底部构建 IP 占用图。"""
        frame = ttk.LabelFrame(parent, text="IP 占用图", padding=8)
        frame.pack(fill=tk.X, pady=(12, 0))

        self.ip_canvas = tk.Canvas(frame, height=160, bg="#f8f9fa", highlightthickness=0)
        self.ip_canvas.pack(fill=tk.X)

        # 图例
        legend_frame = ttk.Frame(frame)
        legend_frame.pack(fill=tk.X, pady=(6, 0))
        for color, label in [("#00b894", "已使用"), ("#dfe6e9", "未使用"), ("#0984e3", "本机"), ("#e17055", "网关")]:
            ...
            c.pack(side=tk.LEFT)
            ttk.Label(row, text=label, font=("", 8)).pack(side=tk.LEFT, padx=(3, 0))

        self.ip_canvas.bind("<Motion>", self._on_ip_map_hover)

    def _update_ip_map(self):
        """根据扫描结果更新 IP 占用图。"""
        self.ip_canvas.delete("all")
        if not hasattr(self, '_current_net') or not self._current_net:
            self.ip_canvas.create_text(120, 40, text="请先扫描", fill="#b2bec3", font=("", 10))
            return

        try:
            network = ipaddress.IPv4Network(self._current_net, strict=False)
        except Exception:
            return

        hosts = list(network.hosts())
        if len(hosts) > 512:
            self.ip_canvas.create_text(120, 40, text="网段过大, 无法显示", fill="#b2bec3", font=("", 10))
            return

        gateway = self._gateway if hasattr(self, '_gateway') else ""
        local_ip = self._current_ip

        canvas_w = self.ip_canvas.winfo_width() or 228
        cols = max(8, canvas_w // 14)
        cell = 12
        gap = 2
        start_x = 4
        start_y = 4

        self._ip_map_rects = {}  # ip -> (x1, y1, x2, y2)
        ...
        for i, host in enumerate(hosts):
            ...
            self.ip_canvas.create_rectangle(x1, y1, x2, y2, fill=color, outline="", tags="cell")
            self._ip_map_rects[ip_str] = (x1, y1, x2, y2)

        # 调整 canvas 高度
        rows = (len(hosts) + cols - 1) // cols
        needed_h = start_y + rows * (cell + gap) + 4
        self.ip_canvas.configure(height=max(needed_h, 80))

        tooltip = getattr(self, '_ip_map_tooltip', None)
        if tooltip:
            self.ip_canvas.delete(tooltip)

    def _on_ip_map_hover(self, event):
        """鼠标悬停显示 IP 地址。"""
        tooltip = getattr(self, '_ip_map_tooltip', None)
        if tooltip:
            self.ip_canvas.delete(tooltip)

        for ip_str, (x1, y1, x2, y2) in getattr(self, '_ip_map_rects', {}).items():
            if x1 <= event.x <= x2 and y1 <= event.y <= y2:
                status = "已使用" if ip_str in self.devices else "未使用"
                self._ip_map_tooltip = self.ip_canvas.create_text(
                    event.x + 10, event.y - 10, text=f"{ip_str} ({status})",
                    anchor=tk.W, fill="#2d3436", font=("", 8),
                    tags="tooltip"
                )
                return
        self._ip_map_tooltip = None

    def _build_result_panel(self, parent):
        # 工具栏
        toolbar = ttk.Frame(parent)
        toolbar.pack(fill=tk.X, pady=(0, 6))
        ttk.Button(toolbar, text="导出 CSV", command=self.export_csv).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="清空结果", command=self.clear_results).pack(side=tk.LEFT, padx=2)

        # 表格
        tree_frame = ttk.Frame(parent)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        columns = ("ip", "mac", "hostname", "os", "dev_type", "status")
        self.tree = ttk.Treeview(tree_frame, columns=columns, show="headings",
                                 selectmode="browse")

        self.tree.heading("ip", text="IP 地址", command=lambda: self._sort_column("ip"))
        self.tree.heading("mac", text="MAC 地址", command=lambda: self._sort_column("mac"))
        self.tree.heading("hostname", text="主机名", command=lambda: self._sort_column("hostname"))
        self.tree.heading("os", text="操作系统", command=lambda: self._sort_column("os"))
        self.tree.heading("dev_type", text="设备类型", command=lambda: self._sort_column("dev_type"))
        self.tree.heading("status", text="状态", command=lambda: self._sort_column("status"))

        self.tree.column("ip", width=130, minwidth=100)
        self.tree.column("mac", width=140, minwidth=110)
        self.tree.column("hostname", width=120, minwidth=80)
        self.tree.column("os", width=130, minwidth=80)
        self.tree.column("dev_type", width=110, minwidth=70)
        ...
        # 标签颜色
        self.tree.tag_configure("self", foreground="#0984e3")
        self.tree.tag_configure("gateway", foreground="#00b894")
        self.tree.tag_configure("normal", foreground="#2d3436")

        self._sort_order = {}

        # 右键菜单
        self.context_menu = tk.Menu(self.tree, tearoff=0)
        self.context_menu.add_command(label="复制 IP 地址", command=self._copy_col_val(0))
        self.context_menu.add_command(label="复制 MAC 地址", command=self._copy_col_val(1))
        self.context_menu.add_separator()
        self.context_menu.add_command(label="Ping 此设备", command=self._ping_selected)
        self.context_menu.add_command(label="查看设备详情", command=self._show_device_detail)
        self.tree.bind("<Button-3>", self._show_context_menu)
        self.tree.bind("<Button-2>", self._show_context_menu)  # 触控板双指

    def _build_status_bar(self):
        frame = ttk.Frame(self.root)
        frame.pack(fill=tk.X, padx=10, pady=(0, 8))
        self.status_var = tk.StringVar(value="就绪 - 请选择网卡后点击「开始扫描」")
        ttk.Label(frame, textvariable=self.status_var).pack(side=tk.LEFT)
        self.stats_var = tk.StringVar(value="")
        ttk.Label(frame, textvariable=self.stats_var).pack(side=tk.RIGHT)

    # ── 网卡加载 ────────────────────────────────────

    def _load_interfaces(self):
        self.networks = get_local_networks()
        if not self.networks:
            self.iface_combo["values"] = ["(未检测到网络接口)"]
            self.iface_combo.current(0)
            return
        values = []
        for iface, ip, net_str in self.networks:
            label = f"{iface} — {ip} ({net_str})"
            values.append(label)
        self.iface_combo["values"] = values
        if values:
            self.iface_combo.current(0)
            self._update_network_info(0)

    def _on_iface_selected(self, event=None):
        idx = self.iface_combo.current()
        if 0 <= idx < len(self.networks):
            self._update_network_info(idx)

    def _update_network_info(self, idx):
        iface, ip, net_str = self.networks[idx]
        self.network_var.set(net_str)
        self.local_ip_var.set(ip)
        self._current_iface = iface
        self._current_ip = ip
        self._current_net = net_str

    # ── 扫描控制 ────────────────────────────────────

    def start_scan(self):
        if not self.networks:
            messagebox.showwarning("警告", "未检测到可用网络接口，无法扫描。")
            return

        idx = self.iface_combo.current()
        if idx < 0 or idx >= len(self.networks):
            return
        iface, local_ip, net_str = self.networks[idx]
        self._current_iface = iface
        ...
        # 获取本机 MAC 和网关
        self._local_mac = get_local_mac(iface, local_ip)
        ...
        total = min(network.num_addresses - 2, 65536)
        self.progress_var.set(0)
        self.progress_bar["maximum"] = total
        self.progress_label["text"] = "正在扫描..."
        self.status_var.set(f"正在扫描 {net_str} ...")

        # 后台线程
        self.scan_start_time = time.time()
        self.scan_thread = threading.Thread(
            target=self._scan_worker,
            args=(net_str, threads),
            daemon=True,
        )
    ...
    def _scan_worker(self, net_str, threads):
        try:
            scan_network(
                net_str,
                max_workers=threads,
                on_device_found=lambda d: self.result_queue.put(("device", d)),
                on_progress=lambda cur, tot, fnd: self.result_queue.put(("progress", cur, tot, fnd)),
    ...
    def _poll_results(self):
        try:
            while True:
                ...
                if mtype == "device":
                    d = msg[1]
                    if d.ip not in self.devices:
                        self.devices[d.ip] = d
                        self._add_device_to_tree(d)
                elif mtype == "progress":
                    _, cur, tot, found = msg
                    self.progress_var.set(cur)
                    self.progress_label["text"] = f"已扫描 {cur}/{tot}，发现 {found} 台设备"
                elif mtype == "error":
    ...
    def _scan_finished(self):
        ...
        if self.stop_flag:
            self.status_var.set(f"已停止 - 发现 {len(self.devices)} 台设备")
            self.progress_label["text"] = "已停止"
            self.stats_var.set(f"在线: {len(self.devices)} 台 | 耗时: {elapsed:.1f}s")
            self._mark_special_devices()
            self._update_ip_map()
        else:
            self.status_var.set(f"扫描完成 - 发现 {len(self.devices)} 台设备，正在刷新 ARP 缓存...")
            self.progress_label["text"] = "刷新 ARP 缓存..."
            self.root.after(200, self._arp_refresh, elapsed)

    def _arp_refresh(self, elapsed):
        """扫描完成后刷新 ARP 缓存，补全缺失的 MAC 和设备信息。"""
        self.status_var.set("正在刷新 ARP 缓存...")
        arp_cache = get_arp_table()
        mac_updated = 0
        info_updated = 0
        for ip, d in self.devices.items():
            # 补全 MAC 地址
            if not d.mac and ip in arp_cache:
                d.mac = arp_cache[ip]
                mac_updated += 1
            # 补全厂商和设备类型 (MAC 可能之前为空, 也可能已有 MAC 但信息缺失)
            if d.mac and (not d.vendor or not d.dev_type):
                d.vendor = mac_to_vendor(d.mac)
                d.dev_type = vendor_device_type(d.vendor) if d.vendor else ""
                if d.dev_type:
                    info_updated += 1
        ...
        for item in self.tree.get_children():
            self.tree.delete(item)
        for d in sorted(self.devices.values(), key=lambda x: ipaddress.IPv4Address(x.ip)):
            self._add_device_to_tree(d)
        self._mark_special_devices()
        self._update_ip_map()
        self.status_var.set(f"扫描完成 - 发现 {len(self.devices)} 台设备")
        parts = []
        if mac_updated:
            parts.append(f"ARP 刷新 {mac_updated} 个 MAC")
        if info_updated and info_updated != mac_updated:
            parts.append(f"补全 {info_updated} 项信息")
        self.progress_label["text"] = f"完成 ({', '.join(parts)})" if parts else "完成"
        self.stats_var.set(f"在线: {len(self.devices)} 台 | 耗时: {elapsed:.1f}s")

    @staticmethod
    def _os_emoji(os_type: str) -> str:
        """将操作系统类型映射为 emoji。"""
        ot = os_type or ""
        if "Windows" in ot: return ""
        if "macOS" in ot or "iOS" in ot: return ""
        if "Linux" in ot: return ""
        if "Android" in ot: return ""
        if "网络设备" in ot: return ""
        return ""

    @staticmethod
    def _dev_type_emoji(dev_type: str) -> str:
        """将设备类型映射为 emoji 图标。"""
        dt = dev_type or ""
        if "路由器" in dt: return "📡"
        if "交换机" in dt: return "🔀"
        if "手机" in dt or "平板" in dt: return "📱"
        if "电脑" in dt or "笔记本" in dt: return "💻"
        if "服务器" in dt: return "🖥"
        if "打印机" in dt: return "🖨"
        if "电视" in dt or "盒子" in dt: return "📺"
        if "摄像头" in dt or "监控" in dt: return "📷"
        if "游戏机" in dt: return "🎮"
        if "物联网" in dt or "智能" in dt: return "🔌"
        if "NAS" in dt or "存储" in dt: return "💾"
        if "音响" in dt or "音频" in dt: return "🔊"
        if "开发板" in dt: return "🔧"
        if "虚拟机" in dt: return "📦"
        if "网络" in dt or "网卡" in dt or "AP" in dt: return "🌐"
        if "代工" in dt: return "🏭"
        return "🖥"

    def _add_device_to_tree(self, d: Device):
        os_emoji = self._os_emoji(d.os_type)
        os_display = f"{os_emoji} {d.os_type}" if d.os_type else os_emoji
        dev_type_display = d.dev_type or "-"
        tag = "normal"
        values = (d.ip, d.mac or "-", d.hostname or "-",
                  os_display, dev_type_display, "在线")
        self.tree.insert("", tk.END, values=values, tags=(tag,))
        count = len(self.tree.get_children())
        self.stats_var.set(f"在线: {count} 台")

    def _mark_special_devices(self):
        gateway = self._gateway
        local_ip = self._current_ip
        for item in self.tree.get_children():
            vals = self.tree.item(item, "values")
            ip = vals[0]
            # columns: ip(0), mac(1), hostname(2), os(3), dev_type(4), status(5)
            if ip == local_ip and ip == gateway:
                self.tree.item(item, values=vals[:5] + ("本机/网关",), tags=("self",))
            elif ip == local_ip:
                self.tree.item(item, values=vals[:5] + ("本机",), tags=("self",))
            elif ip == gateway:
                self.tree.item(item, values=vals[:5] + ("网关",), tags=("gateway",))

    # ── 表格排序 ────────────────────────────────────

    def _sort_column(self, col):
        items = [(self.tree.set(item, col), item) for item in self.tree.get_children("")]
        reverse = self._sort_order.get(col, False)
        if col == "ip":
            try:
                items.sort(key=lambda x: ipaddress.IPv4Address(x[0]), reverse=reverse)
            except Exception:
                items.sort(key=lambda x: x[0], reverse=reverse)
        else:
            items.sort(key=lambda x: x[0].lower(), reverse=reverse)
        self._sort_order[col] = not reverse

        for i, (_, item) in enumerate(items):
            self.tree.move(item, "", i)

        for c in ("ip", "mac", "hostname", "os", "dev_type", "status"):
            arrow = ""
            if c == col:
                arrow = " ▲" if not reverse else " ▼"
            self.tree.heading(c, text=self.tree.heading(c, "text").rstrip(" ▲▼") + arrow)
    ...
    def _copy_col_val(self, col_idx):
        def _copy():
            ...
            if sel:
                ...
                self.status_var.set(f"已复制: {cleaned}")
        return _copy
    ...
    def _ping_selected(self):
        ...
        self.status_var.set(f"正在 Ping {ip} ...")
        threading.Thread(target=self._ping_worker, args=(ip,), daemon=True).start()

    def _ping_worker(self, ip):
        alive = ping_host(ip, timeout=2.0)
        if alive:
            self.root.after(0, lambda: messagebox.showinfo("Ping 结果", f"{ip}\n\nPing 成功，设备在线。"))
            self.root.after(0, lambda: self.status_var.set(f"Ping {ip}: 在线"))
        else:
            self.root.after(0, lambda: messagebox.showwarning("Ping 结果", f"{ip}\n\nPing 超时，设备可能离线或屏蔽了 ICMP。"))
            self.root.after(0, lambda: self.status_var.set(f"Ping {ip}: 超时"))

    def _show_device_detail(self):
        sel = self.tree.selection()
        if not sel:
            return
        vals = self.tree.item(sel[0], "values")
        ip, mac, hostname, os_type, dev_type, status = vals
        detail = (
            f"IP 地址:    {ip}\n"
            f"MAC 地址:   {mac}\n"
            f"主机名:     {hostname}\n"
        ...
        messagebox.showinfo("设备详情", detail)
    ...
    def export_csv(self):
        ...
        if not devices:
            messagebox.showinfo("提示", "没有可导出的数据。请先扫描。")
            return
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        filename = filedialog.asksaveasfilename(
            ...
            initialfile=f"lan_scan_{timestamp}.csv",
        )
        if filename:
            export_csv(devices, filename)
    ...
    def clear_results(self):
        ...
        self.devices.clear()
        self.progress_var.set(0)
        self.progress_label["text"] = "就绪"
        self.stats_var.set("")
        self.status_var.set("结果已清空 - 请点击「开始扫描」")
        if hasattr(self, 'ip_canvas'):
            self._update_ip_map()
